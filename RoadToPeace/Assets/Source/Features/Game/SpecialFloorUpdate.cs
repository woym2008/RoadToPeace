...
public class SpecialFloorUpdate : ReactiveSystem<GameEntity>
{
        ...
        : base(contexts.game)
    {
        ...
        _isbossfighting = false;
    }
    protected override void Execute(List<GameEntity> entities)
    {
        if(_contexts.game.isBossFighting)
        {
            if(!_isbossfighting)
            {
                Debug.LogWarning("GetBossFloorData");
                //先写死
                var datas = _services.SpecialFloorService.GetBossFloorData(0,"LazerShip");
                foreach (var data in datas)
                {
                    var specialfloorEntity = _contexts.game.CreateEntity();
                    specialfloorEntity.ReplaceSpecialFloor(data);
                    Debug.LogWarning("GetBossFloorData" + data.ToString());
                }
                _isbossfighting = true;
            }
        }
        else
        {
    ...
    {
        return context.CreateCollector(GameMatcher.AnyOf( GameMatcher.Difficulty, GameMatcher.BossFighting));
    }
}
